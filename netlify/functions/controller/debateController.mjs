...
export async function createDebate(user, motionId, body) {
	try {
		...
		if (!authorId) 
            return new Response(JSON.stringify({ error: 'authorId required' }), { status: 400, headers: { 'content-type': 'application/json' } });

		const debate = await Debate.create({ motionId, authorId, position, content });
		return new Response(JSON.stringify(debate), { status: 200, headers: { 'content-type': 'application/json' } });
	} catch (err) {
