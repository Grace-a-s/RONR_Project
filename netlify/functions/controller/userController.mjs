...
export async function createUser(_user, body) {
  try {
    ...
    if (!auth0Id)
      return new Response(JSON.stringify({ error: "auth0Id required" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      });
...
export async function updateUser(_user, auth0Id, body) {
  try {
    ...
    if (hasField(body, "firstName"))
      update.firstName = normalizeString(body.firstName);
    if (hasField(body, "lastName"))
      update.lastName = normalizeString(body.lastName);
    if (hasField(body, "pronouns"))
      update.pronouns = normalizeString(body.pronouns);
...
export async function getUsernameById(_user, id) {
  try {
    if (!id) {
